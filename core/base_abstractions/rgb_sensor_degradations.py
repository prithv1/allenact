"""
Python script that houses
degradations to be applied to 
navigation agents

-- [Visual Degradations]
    - In the form of corruptions as of now
    - At the ImageNet level (and not CIFAR-10 level)
"""
# -*- coding: utf-8 -*-
...
def fog(x, severity=1):
    ...
    x += (
        c[0]
        * plasma_fractal(mapsize=256, wibbledecay=c[1])[:256, :256][..., np.newaxis]
    )
...
def frost(x, severity=1):
    ...
    x_start, y_start = (
        np.random.randint(0, frost.shape[0] - 224),
        np.random.randint(0, frost.shape[1] - 224),
    )
...
def snow(x, severity=1):
    ...
    x = c[6] * x + (1 - c[6]) * np.maximum(
        x, cv2.cvtColor(x, cv2.COLOR_RGB2GRAY).reshape(W, W, 1) * 1.5 + 0.5
    )
