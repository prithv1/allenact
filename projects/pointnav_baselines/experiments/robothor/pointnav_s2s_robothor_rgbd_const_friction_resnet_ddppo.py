"""
PointNav RGB-DDPPO Vanilla Experiment Config for Sim2Sim set of experiments
"""
import glob
import os
from abc import ABC
...
from plugins.ithor_plugin.ithor_sensors import RGBSensorThor, DepthSensorThor
...
class PointNavRoboThorRGBPPOExperimentConfig(ExperimentConfig):
    """
    Complete PointNav RoboThor RGB Navigation Experimental Config
    """
    ...
    @classmethod
    def tag(cls):
        return "Pointnav-S2S-RoboTHOR-RGBD-ResNet-DDPPO"
    ...
    def monkey_patch_env_args(
        ...
        if rotation_step_size is not None:
            self.ROTATION_DEGREES = rotation_step_size
        else:
            self.ROTATION_DEGREES = 30

        if translation_noise_std is not None:
            TRANSLATION_NOISE_STD = translation_noise_std
        else:
            TRANSLATION_NOISE_STD = 0.005

        if rotation_noise_std is not None:
            ROTATION_NOISE_STD = rotation_noise_std
        else:
            ROTATION_NOISE_STD = 0.5

        self.ENV_ARGS["gridSize"] = self.STEP_SIZE
        self.ENV_ARGS["rotateStepDegrees"] = self.ROTATION_DEGREES
        self.ENV_ARGS["movementGaussianMu"] = TRANSLATION_NOISE_STD
        self.ENV_ARGS["rotateGaussianMu"] = ROTATION_NOISE_STD
        self.ENV_ARGS["constTranslate"] = const_translate
        self.ENV_ARGS["constRotate"] = const_rotate
    ...
    def monkey_patch_datasets(self, train_dataset, val_dataset, test_dataset):
        ...
        if val_dataset is not None:
            ...
            self.VAL_DATASET_DIR = os.path.join(
                ...
                "datasets/robothor-pointnav/eval_val_0.4_per_sc",
            )
