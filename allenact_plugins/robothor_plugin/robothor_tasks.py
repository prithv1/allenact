...
def soft_progress_metric(d_init: float, d_T: float,) -> Optional[float]:
    if d_init >= 0 and d_T >= 0:
        succ_fac = max(0, 1.0 - (d_T / (d_init + EPS)))
    else:
        succ_fac = 0
    return succ_fac
...
def soft_spl_metric(
    succ_fac: float, optimal_distance: float, travelled_distance: float,
) -> Optional[float]:
    ...
    if succ_fac == 0:
        ...
        travelled_distance = max(travelled_distance, optimal_distance)
        return succ_fac * (optimal_distance / travelled_distance)
...
class PointNavTask(Task[RoboThorEnvironment]):
    ...
    def _step(self, action: Union[int, Sequence[int]]) -> RLStepResult:
        ...
        if action_str == END:
            ...
            if self.env._pyrobot_noise:
                ...
                if action_str == "MoveAhead":
                    teleport_loc, rot_sign, rot_degrees = pyrobot.get_teleport_location(
                        curr_state,
                        self.env.move_step,
                        0.0,
                    ...
                    self.env.step(
                        {"action": rot_action, "degrees": rot_degrees,}
                    )
                    print(
                        action_str, rot_action, rot_degrees,
                    )
    ...
    def metrics(self) -> Dict[str, Any]:
        ...
        soft_progress = soft_progress_metric(
            d_init=self.optimal_distance, d_T=dist2tget,
        )

        soft_spl = soft_spl_metric(
            succ_fac=soft_progress,
            optimal_distance=self.optimal_distance,
            travelled_distance=self.travelled_distance,
        )
